...
- **Final green bar (2026-09-14):** unit np+torch 406, triton 103 (98 + 5 flash), cuda 99 (9 files × 11, per-file subprocess), cross_backend 42, ruff clean, pyright 0 errors (1 pre-existing warning `impl/_triton/attn.py:267`), verify_equivalence 6/6 (greedy match ✓ all three ways).
- **Training-equivalence fix (2026-09-14):** verified training parity with a controlled experiment (same weights/tokens/AdamW lr=1e-3/clip=1.0, 5 steps): torch≡triton≡CUDA to 1e-6, but the NumPy track diverged (0.195 at step 0). Root cause: `scripts/train.py` pre-shifts targets once for every track (standard next-token convention), while `NumPyModel.backward` hardcoded `CrossEntropyLoss()` (default `shift=True`) — the `loss_fn` argument of the NumPy `train_step` only affected the *reported* loss, never the gradients, so the NumPy track trained on a double-shifted target (predicting t+2) and no caller configuration could align it. Fix: unified all tracks on the pre-shifted-target contract (HuggingFace/LLaMA standard) — `NumPyModel.backward` + `NumPyModel._compute_loss` + `impl/_np/gradcheck._loss` + `scripts/train.py` + `scripts/verify_equivalence` now use `CrossEntropyLoss(shift=False)` (the CE class keeps its `shift` feature for teaching). Post-fix parity: numpy vs torch loss 2.2e-5, weight diff 5.1e-5; torch vs triton 1.2e-6 / 1.4e-5. Full green bar re-verified: unit 406, triton 103, cuda 99, cross_backend 42, ruff clean, pyright 0, verify_equivalence 6/6.
