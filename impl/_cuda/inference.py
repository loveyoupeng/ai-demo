...
class CudaTextGenerator:
    ...
    def generate(self, prompt: torch.Tensor) -> torch.Tensor:
        ...
        prompt : torch.Tensor, shape (batch_size, prompt_len)
            Initial token IDs to start from. Must be on the same device
            as the model (CUDA).
    ...
    def generate_greedy(self, prompt: torch.Tensor) -> torch.Tensor:
        ...
        ----------
        prompt : torch.Tensor, shape (batch_size, prompt_len)
            Initial token IDs. Must be on CUDA device.
        ...
        for _ in range(self.max_new_tokens):
            ...
            logits = self.model.forward(sequence)  # pyright: ignore[reportAttributeAccessIssue, reportCallIssue]  # (B, S, V)
...
def _apply_top_k_mask(logits: torch.Tensor, top_k: int) -> torch.Tensor:
    ...
    kth_indices = sorted_indices[:, effective_top_k - 1 : effective_top_k]  # (B, 1)
