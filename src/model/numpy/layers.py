...
class NumPyFeedForward:
    ...
    def backward(
        ...
        N, D = self.x.shape
        grad_w2 = np.dot(
            self.h.reshape(-1, self.dim_ff).T, grad_output.reshape(-1, self.embed_dim)
        )
        ...
        grad_w1 = np.dot(
            self.x.reshape(-1, self.embed_dim).T, grad_z1.reshape(-1, self.dim_ff)
        )
...
class NumPyLayerNorm:
    ...
    def backward(
        ...
        # sum_axes will sum over all dims except the last (features)
        sum_axes = tuple(range(self.x.ndim - 1))
        ...
        grads = {
            "gamma": np.sum(
                grad_output * self.x_norm, axis=sum_axes, keepdims=True
            ).flatten(),
