...
class PyTorchMultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        context_out = context.transpose(1, 2).reshape(batch_size, seq_len, self.embed_dim)
    ...
    def backward(
        ...
        d_W_q = torch.matmul(x.reshape(-1, self.embed_dim).T, d_Q.reshape(-1, self.embed_dim))
        d_W_k = torch.matmul(x.reshape(-1, self.embed_dim).T, d_K.reshape(-1, self.embed_dim))
        d_W_v = torch.matmul(x.reshape(-1, self.embed_dim).T, d_V.reshape(-1, self.embed_dim))
        
        # 8. Gradient w.r.t. input x
        dx = torch.matmul(d_Q, self.W_q.T) + torch.matmul(d_K, self.W_k.T) + torch.matmul(d_V, self.W_v.T)
    ...
    def get_grads(self) -> dict[str, torch.Tensor]:
        return {
            ...
            "qkv.W_v": self.W_v.grad if self.W_v.grad is not None else torch.zeros_like(self.W_v),
            "o.W_o": self.W_o.grad if self.W_o.grad is not None else torch.zeros_like(self.W_o),
        }
