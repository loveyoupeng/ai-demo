        ...
        for name, param in self.model.named_parameters():
            ...
            if name == "token_embedding.embedding.weight":
                ...
                if sub == "ln1":
                    ...
                    # NumPy uses singular 'expert'
                    moe_part = ".".join(parts[3:]).replace("experts", "expert")
            ...
            for ext_name, val in params.items():
                ...
                if ext_name == "token_embedding.weights":
                    ...
                    if sub == "ln1":
                        ...
                        # Convert blocks.0.moe.expert.0.ffn.w1.weight -> layers.0.moe.experts.0.ffn.w1.weight
                        py_name = f"layers.{idx}.moe.{'.'.join(parts[3:]).replace('expert', 'experts')}"
                        parts_moe = py_name.split(".")
                        
                        # Start from the moe module of the layer
                        curr = self.model.layers[idx].moe
                        # parts_moe is ['layers', idx, 'moe', 'experts', '0', 'ffn', 'w1', 'weight']
