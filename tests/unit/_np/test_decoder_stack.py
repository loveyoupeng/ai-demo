...
class TestDecoderStackForward:
    ...
    def test_output_shape(self):
        ...
        stack = DecoderStack(
            TransformerConfig(
                n_layers=2, embed_dim=16, n_heads=4, n_groups=4, n_experts=4, top_k=2, expert_dim=32, seed=0
            )
        )
        out = stack.forward(x)
    ...
    def test_gradient_chaining(self):
        ...
        stack = DecoderStack(
            TransformerConfig(
                n_layers=2, embed_dim=8, n_heads=2, n_groups=2, n_experts=2, top_k=1, expert_dim=16, seed=0
            )
        )

        baseline = stack.forward(x.copy())

        # Perturb first block's input layernorm gamma
        stack.layers[0].input_layernorm.gamma = stack.layers[0].input_layernorm.gamma * 2.0
    ...
    def test_single_layer(self):
        ...
        stack = DecoderStack(
            TransformerConfig(
                n_layers=1, embed_dim=8, n_heads=2, n_groups=2, n_experts=2, top_k=1, expert_dim=16, seed=42
            )
        )
        out = stack.forward(x)
