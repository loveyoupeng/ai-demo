...
class TestFeedForwardParity:
    def setup_method(self):
        ...
        with torch.no_grad():
            for k in ["w1", "b1", "w2", "b2"]:
                getattr(self.pytorch_ffn, k).copy_(
                    torch.from_numpy(getattr(self.numpy_ffn, k))
                )
    ...
    def test_forward_parity(self):
        ...
        np.testing.assert_allclose(
            numpy_out, pytorch_out.detach().numpy(), rtol=1e-5, atol=1e-5
        )
    ...
    def test_backward_w1_parity(self):
        ...
        np.testing.assert_allclose(
            numpy_grads["w1"],
            pytorch_grads["ffn.w1"].detach().numpy(),
            rtol=1e-5,
            atol=1e-5,
        )
    ...
    def test_backward_w2_parity(self):
        ...
        np.testing.assert_allclose(
            numpy_grads["w2"],
            pytorch_grads["ffn.w2"].detach().numpy(),
            rtol=1e-5,
            atol=1e-5,
        )
    ...
    def test_backward_b1_parity(self):
        ...
        np.testing.assert_allclose(
            numpy_grads["b1"],
            pytorch_grads["ffn.b1"].detach().numpy(),
            rtol=1e-5,
            atol=1e-5,
        )
    ...
    def test_backward_b2_parity(self):
        ...
        np.testing.assert_allclose(
            numpy_grads["b2"],
            pytorch_grads["ffn.b2"].detach().numpy(),
            rtol=1e-5,
            atol=1e-5,
        )
    ...
    def test_backward_x_parity(self):
        ...
        np.testing.assert_allclose(
            dx_numpy, dx_pytorch.detach().numpy(), rtol=1e-5, atol=1e-5
        )
