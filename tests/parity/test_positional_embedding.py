...
class TestPositionalEmbeddingParity:
    ...
    def test_pe_matrix_parity(self):
        ...
        np.testing.assert_allclose(
            self.numpy_pe.pe, self.pytorch_pe.pe.detach().numpy(), rtol=1e-6, atol=3e-6
        )
    ...
    def test_backward_x_parity(self):
        ...
        np.random.seed(42)
        x = np.random.randn(4, self.max_seq_len, self.embed_dim).astype(np.float32)
        grad_output = np.random.randn(4, self.max_seq_len, self.embed_dim).astype(
            np.float32
        )
    ...
    def test_short_sequence_parity(self):
        ...
        np.testing.assert_allclose(
            pe_slice_numpy, pe_slice_pytorch.detach().numpy(), rtol=1e-6, atol=1e-6
        )
