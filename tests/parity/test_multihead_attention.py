...
class TestMultiHeadAttentionParity:
    ...
    def test_forward_parity(self):
        ...
        pytorch_out, self.pytorch_cache = self.pytorch_mha.forward(
    ...
    def test_backward_q_parity(self):
        ...
        _, numpy_grads = self.numpy_mha.backward(
            x, grad_output, mask, **{k: v for k, v in cache.items() if k != "x"}
        )

        self.pytorch_mha.forward(torch.from_numpy(x), torch.from_numpy(mask))
        _, pytorch_grads = self.pytorch_mha.backward(
            torch.from_numpy(grad_output), torch.from_numpy(mask)
        )

        np.testing.assert_allclose(
            numpy_grads["W_q"],
            pytorch_grads["qkv.W_q"].detach().numpy(),
            rtol=1e-4,
            atol=1e-4,
        )
    ...
    def test_backward_k_parity(self):
        ...
        np.testing.assert_allclose(
            numpy_grads["W_k"],
            pytorch_grads["qkv.W_k"].detach().numpy(),
            rtol=1e-4,
            atol=1e-4,
        )
    ...
    def test_backward_v_parity(self):
        ...
        np.testing.assert_allclose(
            numpy_grads["W_v"],
            pytorch_grads["qkv.W_v"].detach().numpy(),
            rtol=1e-4,
            atol=1e-4,
        )
    ...
    def test_backward_o_parity(self):
        ...
        np.testing.assert_allclose(
            numpy_grads["W_o"],
            pytorch_grads["o.W_o"].detach().numpy(),
            rtol=1e-4,
            atol=1e-4,
        )
    ...
    def test_backward_x_parity(self):
        ...
        dx_numpy, _ = self.numpy_mha.backward(
            x, grad_output, mask, **{k: v for k, v in cache.items() if k != "x"}
        )

        self.pytorch_mha.forward(torch.from_numpy(x), torch.from_numpy(mask))
        dx_pytorch, _ = self.pytorch_mha.backward(
            torch.from_numpy(grad_output), torch.from_numpy(mask)
        )

        np.testing.assert_allclose(
            dx_numpy, dx_pytorch.detach().numpy(), rtol=1e-4, atol=1e-4
        )
